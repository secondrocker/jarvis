...
def _prepare_pdf_input(context: ExecutionContext) -> dict:
    ...
    ``pdf_base64`` 缺省时把 ``message`` 视为可下载的 PDF URL。
    """
    payload = {**context.parameters, "source": context.message}
...
def create_workflows(*, settings: Settings) -> dict[str, ExecutorDefinition]:
    ...
    pdf_to_image = WorkflowExecutor(
        workflow=build_pdf_to_image_graph(output_dir=Path(settings.pdf_image_output_dir)),
    ...
    return {
        ...
        "pdf_to_image": ExecutorDefinition(
            mode=SelectedMode.WORKFLOW,
            description="Render PDF pages to images served via static files (no LLM)",
